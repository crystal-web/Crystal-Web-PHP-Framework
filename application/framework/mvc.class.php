...
public $totalduration;
	...
	{
	$this->action++;
	$this->memoryStart = (function_exists('memory_get_usage') ? memory_get_usage() : 0 );
	$this->timeStart=getmicrotime();
	}
	
	public function stop()
	{
	$actualTime = getmicrotime()-$this->timeStart;
		$this->totalduration += $actualTime;
	
	$actualMemory = ($this->memoryStart - (function_exists('memory_get_usage') ? memory_get_usage() : 0 ) );
		$this->totalMemory += $actualMemory;
	return array('time' => $actualTime, 'size' => $actualMemory);
	}
	
	public function getChrono()
	{
	return 'Page g&eacute;n&eacute;r&eacute; en ' . round($this->totalduration,5) . ' sec<br />' . 
	$this->action . ' &eacute;venements<br />
	M&eacute;moire utilis&eacute;s ' . $this->convert($this->totalMemory).'<br />';
	}
	...
	public function convert($size)
	{
		...
		return ($size>0) ? @round($size/pow(1024,($i=floor(log($size,1024)))),2). ' ' . $unit[$i] : 0;
	}
}
