            ...
            //$this->mvc->Session->user('id'));
                if ($calendar->addEvent($this->mvc->Request->data))
                {
                    ...
                    ***************************************/
                    ...
                    $qui = $this->mvc->Session->user('login');
                    $data_arr = explode('-', $this->mvc->Request->data->date);
                    list($year, $month, $day) = $data_arr;
                    $event->push($qui . ' a ajouté un nouvel évenment au calendrier a la date du <a href="'.Router::url('calendar/event/year:'.$year.'/month:'.$month.'/day:'.$day.'').'">'.dates(mktime(0, 0, 0, $month, $day, $year), 'fr_date').'</a>');
