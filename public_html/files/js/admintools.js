...
$.editableFactory = {
	...
	'editor': {
		toEditable: function($this,options){
			$('<textarea name="' + $this[0].id + '"/>').appendTo($this)
							.val($this.data('editable.current'));
			CKEDITOR.replace($this[0].id, {"toolbar":[[ "Source","-","Save","NewPage","Preview","Print","-","Templates" ], [ "Find","Replace","-","SelectAll","-","SpellChecker", "Scayt","-","Maximize" ], ["Undo","Redo","-","Cut","Copy","Paste","PasteText","PasteFromWord"], [ "Undo","Redo","-","Bold","Italic","Underline","Strike","NumberedList","BulletedList","-","Outdent","Indent","-","Blockquote","CreateDiv","-","JustifyLeft","JustifyCenter","JustifyRight","JustifyBlock","-","BidiLtr","BidiRtl" ], [ "Image","Flash","Table","HorizontalRule","Smiley","SpecialChar","PageBreak","Iframe" ], [ "Link","Unlink","Anchor" ], [ "TextColor","BGColor" ], [ "Styles","Format","Font","FontSize" ]]});
		},
		getValue: function($this,options){
			return $this.children().val();
		}
